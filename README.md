...
### 3. 상태 제어창 모듈 (`Status.tsx`)
* **직관적인 전체 드래그 이동**: 최상단 헤더 영역뿐 아니라 창 내부의 빈 영역 어느 곳이나 자유롭게 드래그하여 배치할 수 있는 편의성 극대화.
* **이벤트 전파 격리 처리**: 레벨업/경험치 조작, 크기 슬라이더 세밀 조정, 펫 색상 변경 버튼 클릭 시 마우스 드래그 액션이 오작동을 야기하지 않도록 완벽하게 포인터 이벤트 전파(`stopPropagation`) 제어.
* **펫 투명도 및 레이아웃 커스텀**: 단지 상태 확인뿐만 아니라 슬라이더 터치 한 번으로 캐릭터의 실시간 스케일링 및 색상 보정이 가능한 직관적인 UI 제공.

### 4. 달력 및 할 일 리스트 모듈 (`Calendar.tsx`, `Todo.tsx`)
* **그리드 달력**: 월별 달력 안에서 스케줄 추가, 조회, 수정 및 제거 기능 수행.
* **드래그 & 잠금 기능**: 달력을 마우스 드래그로 손쉽게 배치할 수 있고, 잠금 아이콘을 활성화하여 클릭 실수에 의한 의도치 않은 드래그 방지 지원.
...
프로젝트 루트 폴더 내에 `.env.local` 파일을 생성하고 구글 제미나이 API 키 변수를 넣어줍니다.
```env
